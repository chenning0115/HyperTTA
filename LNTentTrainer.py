...
from models import ssftt_new as SSFTT
from models import SQS
from models import transformer_ori 
import utils
...
def collect_params(model):
    ...
    for nm, m in model.named_modules():
        if isinstance(m, nn.LayerNorm):  # original: nn.BatchNorm1d, nn.BatchNorm2d, nn.LayerNorm, nn.GroupNorm
            # for np, p in m.named_parameters():
            #     if np in ['weight', 'bias'] and p.requires_grad:  # weight is scale, bias is shift
            #         params.append(p)
            #         names.append(f"{nm}.{np}")
            if hasattr(m, 'weight') and m.weight.requires_grad:  # and m.weight.requires_grad
                params.append(m.weight)
                names.append(f"{nm}.weight")
            if hasattr(m, 'bias') and m.weight.requires_grad:
                params.append(m.bias)
                names.append(f"{nm}.bias")
    return params, names
...
def configure_model(model_0):
    ...
    # Enable gradients only for LayerNorm layers
    for m in model.modules():
        if isinstance(m, nn.LayerNorm):
            m.requires_grad_(True)  # Enable gradients for LayerNorm layers
...
class TransformerOriTrainer(BaseTrainer):
    def __init__(self, params):
        super(TransformerOriTrainer, self).__init__(params)
        self.lr = None
        self.weight_decay = None

    def real_init(self):
        # net
        self.net = transformer_ori.TransFormerNet(self.params).to(self.device)
        # loss
        self.criterion = nn.CrossEntropyLoss()
        # optimizer
        self.lr = self.train_params.get('lr', 0.001)
...
class SQSTrainer(BaseTrainer):
    def __init__(self, params):
        super(SQSTrainer, self).__init__(params)
        self.lr = None
        self.weight_decay = None

    def real_init(self):
        # net
        self.net = SQS.SQSFormer(self.params).to(self.device)
        # loss
        self.criterion = nn.CrossEntropyLoss()
        # optimizer
        self.lr = self.train_params.get('lr', 0.001)
...
class SSFTTNEWTrainer(BaseTrainer):
    def __init__(self, params):
        super(SSFTTNEWTrainer, self).__init__(params)
        self.lr = None
        self.weight_decay = None

    def real_init(self):
        # net
        self.net = SSFTT.SSFTTnet(self.params).to(self.device)
        # loss
        self.criterion = nn.CrossEntropyLoss()
        # optimizer
        self.lr = self.train_params.get('lr', 0.001)
